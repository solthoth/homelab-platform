...
- **`machine.certSANs` and `cluster.apiServer.certSANs` are different fields — do not confuse them.** The former only extends Talos's own apid/machined API cert (port 50000); only the latter extends the kube-apiserver serving cert (port 6443), which is what `kubectl` actually TLS-verifies against. Adding a LAN/remote address to the wrong one leaves `talosctl` working while `kubectl` fails TLS verification — a confusing split to debug. Set via `talos-bootstrap.ps1 -ApiServerCertSANs`; see `kubernetes/README.md`'s "Remote cluster access" section.
- **New kubectl users/devices get a scoped credential via `new-cluster-user.ps1`, never a copy of the admin `kubeconfig`.** It mints a per-identity cert through Kubernetes' own CertificateSigningRequest API (no OIDC needed) and a matching `ClusterRoleBinding`, so `revoke-cluster-user.ps1` can pull one person's access independently of everyone else's. The one sharp edge: revoking removes *authorization* only — native Kubernetes client-cert auth has no revocation list, so the cert itself stays valid until `-ExpirationDays` (default 90) runs out. Both scripts' header comments spell this out; read them before treating this as equivalent to OIDC/SSO.
