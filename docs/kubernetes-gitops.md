...
The Kubernetes API has the same LAN-reachability problem as any other in-cluster service, bridged the same way: a host-side PowerShell script (`infrastructure/hyper-v/k8s-api-bridge.ps1`) forwards the Hyper-V host's LAN IP, port 6443, to the control-plane node's static address. The part that needs more than a port forward is TLS — the API server's serving certificate only carries SANs Talos already knows about, so the LAN IP has to be added explicitly via `cluster.apiServer.certSANs` (set through `talos-bootstrap.ps1 -ApiServerCertSANs`, distinct from — and easy to confuse with — `machine.certSANs`, which covers Talos's own API, not Kubernetes'). See `kubernetes/README.md`'s "Remote cluster access" section for the full runbook, including copying and re-pointing `kubeconfig` from Linux/macOS or Windows.

Rather than handing every new device a copy of the cluster-admin `kubeconfig`, `infrastructure/hyper-v/new-cluster-user.ps1` mints a scoped, individually-revocable one per person/device using Kubernetes' own CertificateSigningRequest API (no OIDC/identity-provider infrastructure needed — just the cluster's own CA, which Talos already wires up for it). Each identity gets its own `ClusterRoleBinding`, so `revoke-cluster-user.ps1` can pull one person's access without touching anyone else's. The one real caveat, spelled out in both scripts' header comments: native Kubernetes client-cert auth has no certificate revocation list, so "revoke" removes authorization, not the certificate's validity — a short `-ExpirationDays` is the actual backstop.
